...
export abstract class ExpedtitionRepository{
  ...
  abstract filter(listExpeditionDTO: ListExpeditionDTO, limit: number, offset: number):Promise<Expedition[]>
  abstract list(limit: number, offset: number): Promise<Expedition[]>
  abstract updateStatusAndTruck(id: string, updateExpeditionDto: UpdateExpeditionDto):Promise<void>
}