...
export class UpdateStatusAndTruckService {
  constructor(
    private expeditionRepository: ExpedtitionRepository,
  ...
  async execute(id: string, { truck_code }: UpdateExpeditionDto) {
    ...
    if (expedition.fk_truck) {
      throw new HttpException(
        'Caminhão já vinculado a esta expedição',
    ...
    return await this.expeditionRepository.updateStatusAndTruck(id, {
      fk_truck,
      fk_status,
    });
  }
